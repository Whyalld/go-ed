...
func main() {
	circle, _ := NewCircle(42)
	rectangle, _ := NewRectangle(23, 32)

	fmt.Println("Площадь круга:", CalculateArea(circle))
	fmt.Println("Периметер круга:", CalculatePerimеter(circle))

	fmt.Println("\nПлощадь прямоугольника:", CalculateArea(rectangle))
	fmt.Println("Периметер прямоугольника:", CalculatePerimеter(rectangle))
}